...
from processdata.collate_functions import collate_to_max_length
from processdata.ner_dataset import NERDataset
from models.modeling_glycebert import GlyceBertForTokenClassification
from utilss.random_seed import set_random_seed
...
class NERTask(pl.LightningModule):
    def __init__(self, args: argparse.Namespace):
        ...
        self.bert_config = BertConfig.from_pretrained(self.bert_dir, output_hidden_states=True, return_dict=True,
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self.compute_loss(model_output.logits, labels, loss_mask=loss_mask)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        pinyin_ids = pinyin_ids.view(batch_size, seq_len, 8)
        model_output = self.forward(input_ids=input_ids, pinyin_ids=pinyin_ids,)
        loss = self.compute_loss(model_output.logits, gold_labels, loss_mask=loss_mask)
        probabilities, argmax_labels = self.postprocess_logits_to_labels(model_output.logits.view(batch_size, seq_len, -1))
...
def evaluate():
    ...
    model = NERTask.load_from_checkpoint(checkpoint_path=args.checkpoint_path,
